...
def dataset_prep():
    if os.path.exists(filename):
        ...
        train_rd = np.vstack((df_rd,df_rd_et)).astype(np.float16)
        save(filename_ds, 'train_rd', train_rd)
        df_r = load(filename,ct_r,mode)
        df_r_et = load(filename,et_r,mode)
        train_r = np.vstack((df_r,df_r_et)).astype(np.float16)
        save(filename_ds, 'train_r', train_r)
        labels = np.hstack((ct_labels,et_labels)).astype(np.int8)
